...
(current-material '(0.0 0.1 0.9 0.1))
...
(define (make-segment n)
  ...
  (combine (scale (ellipsoid '(-1/2 -1/2 0) '(1/2 1/2 1))
           ...
           (basis 'bottom (point-at '(0 0 0) '(0 0 1)))
           (if (zero? (modulo n 3))
               (combine (basis 'top (point-at (list 0 0 size) (list 1 0 1) 45))
                        (basis 'top (point-at (list 0 0 size) (list -1 0 1) -45)))
               (basis 'top (point-at (list 0 0 size)
                                     (list (if (zero? (modulo n 2)) -1 1) 0 1/2)
                                     0)))))
  ...
  (when (send frame is-shown?)
    (let ([chain  (rotate-z (wiggle chain) 0.5)])
      ...
      (send canvas set-pict3d
            (combine (basis 'camera (point-at '(0 2 4) '(0 -1/2 -1)))
