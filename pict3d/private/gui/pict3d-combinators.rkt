...
(define (map-group/transform p n f)
  (if (empty? n)
      ...
      (scene-map-group/transform (pict3d-scene p) n (λ ([t : Affine] [s : group-scene])
                                                      (f t (pict3d s))))))
...
(define (interpret-vertex vert cc ce cm)
  (cond [(pos? vert)  (values (vec-flvector vert) #f cc ce cm)]
        [else
         (match-define (Vertex v n c e m) vert)
         (values (vec-flvector v)
                 (if n (vec-flvector n) #f)
                 (if c (col-flvector c) cc)
                 (if e (col-flvector e) ce)
                 (if m m cm))]))
...
(define (triangle vert1 vert2 vert3 #:back? [back? #f])
  (define cc (col-flvector (current-color)))
  (define ce (col-flvector (current-emitted)))
...
(define (rectangle v1 v2 #:inside? [inside? #f])
  (let ([v1  (vec-flvector v1)])
    (define b
      (cond [(pos? v2)  (assert (flv3rect (vector v1 (vec-flvector v2))) nonempty-flrect3?)]
            [else
             (define r
               (cond [(real? v2)  (define r (fl v2))
                                  (flvector r r r)]
                     [else  (vec-flvector v2)]))
             (assert (flv3rect (vector (flv3- v1 r) (flv3+ v1 r))) nonempty-flrect3?)]))
    (pict3d
     (shape->scene
      (make-rectangle-shape b
                            (col-flvector (current-color))
                            (col-flvector (current-emitted))
                            (current-material)
                            (and inside? #t))))))
...
(define (sunlight dv [e  (emitted 1.0 1.0 1.0 1.0)])
  (let ([dv  (flv3normalize (vec-flvector dv))])
    (if dv
        (pict3d (shape->scene (make-directional-light-shape (col-flvector e) dv)))
        empty-pict3d)))
...
(define scale
  (make-transformer (λ ([v : (U Real Dir)])
                      (scale-flt3 (check-scale 'scale (if (real? v) (fl v) (vec-flvector v)))))))
...
(define move (make-transformer (λ ([v : Dir]) (translate-flt3 (vec-flvector v)))))
...
(define (check-axis name orig-v)
  (define v (flv3normalize (vec-flvector orig-v)))
  (if v v (raise-argument-error name "nonzero axis vector" v)))

(define rotate-x (make-transformer (λ ([a : Real])
                                     (rotate-flt3 (vec-flvector +x) (degrees->radians (fl a))))))
(define rotate-y (make-transformer (λ ([a : Real])
                                     (rotate-flt3 (vec-flvector +y) (degrees->radians (fl a))))))
(define rotate-z (make-transformer (λ ([a : Real])
                                     (rotate-flt3 (vec-flvector +z) (degrees->radians (fl a))))))
...
(define (point-at from to 
  ...
  (affine (point-at-flt3 (vec-flvector from)
                         (vec-flvector z-axis)
                         (degrees->radians (fl angle))
                         (vec-flvector up)
                         (and normalize? #t))))
...
(define (standard-cylinder-scene inside? n)
  (define c (col-flvector (current-color)))
  (define e (col-flvector (current-emitted)))
