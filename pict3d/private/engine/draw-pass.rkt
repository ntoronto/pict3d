...
(define (send-transform-data tbuf indexed? ps start end vertex-count)
  ...
  (for/fold ([vertex-num : Nonnegative-Fixnum  0]) ([i  (in-range start end)])
    ...
    (cond
      [(eq? indexed? (and (vertices-indexes v) #t))
       (define vertex-count (vertices-vertex-count v))
       (define transform-data (affine-data (draw-params-affine p)))
       (memcpy* tmp-transform-data-ptr (unsafe-fx* vertex-num affine-data-size)
        ...
        (for ([ks  (in-list ((inst group-by-key! draw-params Face)
                             ...
                             (λ ([ts : draw-params])
                               (cond [(shape-params-two-sided? (draw-params-shape-params ts))  'both]
                                     [(affine-consistent? (draw-params-affine ts))  face]
  ...
  (let ([get-params : (-> passes (Vectorof shape-params))  get-params-stx])
    (λ ([passess : (Vectorof draw-passes)]
      ...
      (for ([i  (in-range n)])
        ...
        (set-draw-params-affine! pi identity-affine)))))
